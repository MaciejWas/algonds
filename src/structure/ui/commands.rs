...
impl UIElement for CommandsView {
    ...
    fn setup(view: &View) -> Self {
        ...
        Self { selected, compile_command, run_command }
    }
    ...
    fn render<B: Backend> (self, frame: &mut Frame<B>, layout: &ProblemScreenLayout) {
        ...
        let commands = Paragraph::new(
            vec![
                Spans::from(Span::styled("[C]ompile command: ".to_string() + &self.compile_command, compile_style)),
                Spans::from(Span::styled("[R]un Command: ".to_string() + &self.run_command, run_style)),
            ]
        );
