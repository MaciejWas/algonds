...
impl TestCaseIO {
    ...
    pub fn get_io(&self) -> Result<(Stdio, Stdio), String> {
        let stdout: Stdio = self.stderr_file.try_clone().map_err(to_string)?.into();
    ...
    pub fn get_stdout(&mut self) -> String {
        ...
        self.stdout_file.read_to_string(&mut buf).unwrap();
        buf
    }

    pub fn get_stderr(&mut self) -> String {
        let mut buf = String::new();
        self.stderr_file.read_to_string(&mut buf).unwrap();
        buf
    }
}
