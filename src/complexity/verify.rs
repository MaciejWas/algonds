...
fn verify(expected_complexity: Box<dyn Function>, x: Vec<u32>, y: Vec<u32>) -> Result<f64, linreg::Error> {
    let codomain: Vec<u32> = y.iter().map(|y| expected_complexity.closest_inverse(y)).collect();
    ...
    Ok(zip(test_x, test_y).into_iter()
        .map(|(x, y)| find_err(intercept + x as f64 * slope, y as f64))
        .min_by(|a, b| a.partial_cmp(b).unwrap()).unwrap()) // TODO: Remove unwrap
